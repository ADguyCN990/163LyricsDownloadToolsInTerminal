...
| 双语歌词 | 支持原文和译文合并保存 |
| 逐字歌词 | 支持逐字时间戳，每行内每个字都有独立时间戳（-w参数） |
| LRC格式 | 标准LRC歌词文件输出 |
| 错误重试 | API限流时自动重试 |
...
# 下载逐字歌词（每行内每个字都有独立时间戳，需要歌曲支持）
python lyric_cli.py "https://music.163.com/#/song?id=347230" -o ./lyrics -w
...
-w, --word-by-word  输出带逐字时间戳的歌词（每行内每个字都有独立时间戳）
```
...
### lyric_cli.py 输出（逐字时间戳LRC）

使用 `-w` 参数可生成带逐字时间戳的歌词，支持播放器逐字高亮显示：
...
[00:00.00]<00:00.00>在<00:00.25>希<00:00.45>望<00:00.60>的<00:00.80>田<00:01.00>野<00:01.20>上
[00:05.20]<00:05.20>所<00:05.40>有<00:05.60>的<00:05.80>评<00:06.00>判<00:06.20>都<00:06.40>失<00:06.60>去
[00:09.50]<00:09.50>了<00:09.70>意<00:09.90>义
```
